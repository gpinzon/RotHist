...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%								       %%
%% Program : history_v15_60Mo.c      				       %%
%% REFUGEE ( Rotational historiEs oF yoUnG stEllar objEcts )           %%
%% -.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-..-.-.-..-.-.-..-.-.-.-%%
%% Rotation evolution for a young M*=6.00Mo star                       %%       
%% -.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-..-.-.-..-.-.-..-.-.-.-%%
%% APR 2021  						               %%
...
#include <ctype.h>
#include <unistd.h>
#define PRECISION 0.0001  /* in Sean's Rt/Rco Solver */
...
#define NPMODEL 432 /* Number of lines Siess Model files for 1Mo*/
...
FRAC=6.0;
 ...
 //MACC= ( 2.7941 * log10(FRAC) ) - 7.739;   /*usando solamente fairlamb2015table3*/
...
*/
 double taoi,efecin,taofinal,mucero,turnover,mamon;
  ...
  double C_M = 0.001;  // timestep factor, sets max change in Mstar
  double C_Z = 0.001;  // timestep factor, sets max change in Rstar.
  double C_A = 0.001;  // timestep factor, sets max change in Omegastar.
...
//   
     ...
     inertiacore=kdosrad*muc*pow(radc,2.0);
    inertiaenv=kdos*mui*pow(zeti,2.0);
//   
...
//     Calculate timestep for changes in radius.
    ...
    stepa = (torque == 0.0) ? 1.0e20 :
      fabs(C_A * alphai * omegasun  *mui*MSUN*pow(RSUN*zeti,2.0) / 
	   torque)/tg;
...
// return tg*torque/(kdos*mui*MSUN*pow(RSUN*zeti,2.0)*omegasun) - alphai*(muiprima/mui + 2.*dzdt/zeti);
...
double dalpha_dtaucore(double torqueoncore, double taoc,double inertiacore, double radc, double muc, double alphai, double dinertiacoredt, double alphaicore,  double kdosrad, double mc, double dzdtcore, double tg, double zeti)
{
return -taoc*NSA*torqueoncore/(0.2*muc*MSUN*pow(RSUN*radc,2.0)*omegasun)+
...
double BE(double alphai,double zeti,double BCERO, double turnover)
{
 ...
 // magnetic=BCERO*turnover*(alphai/6.28)/40.0;
 magnetic = BCERO;
...

{
 ...
 // readm = fopen("k2ov1.csv","r"); /* 209 puntos */
  readmdtres = fopen("m6.0z02.hrd","r"); 
  //ojo:
...

{
 ...
 // readm = fopen("k2ov1.csv","r"); /* 209 puntos */
  readm264 = fopen("natalia12.dat","r"); 
